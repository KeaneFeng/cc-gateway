...
pub fn show_usage(db: &Database, days: i32) -> anyhow::Result<()> {
    ...
    println!("  {:<25} {:>15}", "Input Tokens", format_number(summary.total_input_tokens));
    println!("  {:<25} {:>15}", "Output Tokens", format_number(summary.total_output_tokens));
    println!("  {:<25} {:>15}", "Total Cost", format!("${:.4}", summary.total_cost_usd));
    println!("  {:<25} {:>15}ms", "Avg Latency", summary.avg_latency_ms);
    println!("  ────────────────────────────────────────────────────────────────────────");
    
    // Get daily breakdown
    let end_date = Utc::now().format("%Y-%m-%d").to_string();
    let start_date = (Utc::now() - Duration::days(days as i64)).format("%Y-%m-%d").to_string();
...
pub fn show_provider_usage(db: &Database, provider_id: &str, days: i32) -> anyhow::Result<()> {
    ...
    let stats = db.get_usage_stats(app_type, &start_date, &end_date)?;
    let provider_stats: Vec<_> = stats.iter().filter(|s| s.provider_id == provider_id).collect();
    ...
    println!("\n  {} Usage for Provider: {}", style("📊").cyan(), style(provider_id).green());
    println!("  ────────────────────────────────────────────────────────────────────────");
    println!("  {:<25} {:>15}", "Total Requests", total_requests);
    println!("  {:<25} {:>15}", "Input Tokens", format_number(total_input));
    println!("  {:<25} {:>15}", "Output Tokens", format_number(total_output));
    println!("  {:<25} {:>15}", "Total Cost", format!("${:.4}", total_cost));
    println!("  ────────────────────────────────────────────────────────────────────────");
