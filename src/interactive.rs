...
use dialoguer::{Select, Input, Confirm, FuzzySelect, MultiSelect};
    ...
    loop {
        ...
        match selection {
            ...
            9 => {
                ...
                println!("Run: cc-switch-pro serve");
                break;
            }
            10 => {
                println!("Goodbye!");
...
fn print_header(term: &Term) -> anyhow::Result<()> {
    ...
    term.write_line(&format!(
        "{}",
        style("║           CC-Switch-Pro - Provider Manager               ║").cyan()
    ))?;
    term.write_line(&format!(
        "{}",
        style("╚═══════════════════════════════════════════════════════════╝").cyan()
...
fn list_providers_interactive(config_path: &PathBuf) -> anyhow::Result<()> {
    ...
    for (i, p) in config.providers.iter().enumerate() {
        ...
        term.write_line(&format!(
            ...
            truncate(&p.id, 14),
            truncate(&p.name, 24),
            format!("claude-{}", truncate(&p.id, 10)),
            default
        ))?;
    }
    ...
    // Let user select a provider to view details
    ...
    let selection = Select::new()
        .with_prompt("Select provider to view details (or Back)")
        .items(&items)
        .default(0)
        .interact()?;
    ...
    if selection < config.providers.len() {
        ...
        term.write_line(&format!("  API Key:     {}...", &p.api_key[..std::cmp::min(8, p.api_key.len())]))?;
        if let Some(ref dn) = p.display_name {
            term.write_line(&format!("  Display:     {}", dn))?;
        }
...
fn add_from_preset_interactive(config_path: &PathBuf) -> anyhow::Result<()> {
    ...
    let mut category_items: Vec<String> = categories
        .iter()
        .map(|c| format!("{} ({})", presets::get_category_display_name(c), presets::get_presets_by_category(c).len()))
        .collect();
    category_items.push("← Back".to_string());
    ...
    let presets = presets::get_presets_by_category(selected_category);
    ...
    let selected_preset = &presets[preset_selection];
    ...
    let custom_id: String = Input::new()
        .with_prompt("Custom ID (leave empty for default)")
...
fn add_custom_interactive(config_path: &PathBuf) -> anyhow::Result<()> {
    ...
    let id: String = Input::new()
        .with_prompt("Provider ID (e.g., myprovider)")
        .interact_text()?;
...
fn edit_provider_interactive(config_path: &PathBuf) -> anyhow::Result<()> {
    ...
    let display_name: String = Input::new()
        ...
        .default(provider.display_name.clone().unwrap_or_default())
        .interact_text()?;
...
fn copy_provider_interactive(config_path: &PathBuf) -> anyhow::Result<()> {
    ...
    let selection = Select::new()
        .with_prompt("Select provider to copy")
        .items(&items)
        .default(0)
        .interact()?;
    ...
    let new_id: String = Input::new()
        .with_prompt("New provider ID")
        .default(format!("{}-copy", source_id))
        .interact_text()?;
...
fn list_presets_interactive() -> anyhow::Result<()> {
    ...
    loop {
        ...
        let preset_items: Vec<String> = presets_list
            .iter()
            .map(|p| format!("{:<20} {:<30} {}", p.id, p.name, p.display_name))
            .collect();

        term.write_line(&format!(
            "\n  {} presets:",
            presets::get_category_display_name(selected_category)
        ))?;
        term.write_line("  ────────────────────────────────────────────────────────────────────────")?;
        term.write_line(&format!(
            "  {:<20} {:<30} {}",
...
fn import_interactive(config_path: &PathBuf) -> anyhow::Result<()> {
    ...
    term.write_line(&format!(
        "  Found cc-switch database: {}",
        cc_switch_db.display()
    ))?;

    let confirm = Confirm::new()
        .with_prompt("Import providers from cc-switch?")
        .default(true)
        .interact()?;
    ...
    term.write_line(&format!(
        "\n  {} Imported {} providers from cc-switch!",
        style("✓").green(),
        imported_count
    ))?;
    term.write_line("  Press Enter to continue...")?;
